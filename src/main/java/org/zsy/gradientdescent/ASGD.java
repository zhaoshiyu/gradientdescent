...
public class ASGD {
	...
	public static double[] updateTheta(double[] theta, double[] x, double y, double alpha) throws Exception {
		...
		for(int i = 0; i < len; i++) {
			for(int j = 0; j < len; j++) {
				if (i == 0) {//theta_0计算稍微有点差别，这里到没差别，因为batchX[i][0]的值为1
					gradient += dev;
				} else {
					gradient += dev * x[j];
				}
	...
	public static double[] updateTheta(double[] theta, double[][] batchX, double[] batchY, double alpha) throws Exception {
		...
		for(int i = 0; i < batchSize; i++) {
			...
			for(int j = 0; j < parameter; j++) {
				for(int k = 0; k < parameter; k++) {
					if (j == 0) {//theta_0计算稍微有点差别，这里到没差别，因为batchX[i][0]的值为1
...
//		}
	 ...
	 * @throws Exception
	 */
	public static double[] asgdTrain(double[][] x_data, double[] y_data, double alpha, double deviation, int batchSize, int iterations) throws Exception {
		int parameter = x_data[0].length;
		int size = y_data.length;
		if (size % batchSize != 0) {
			throw new Exception("样本数必须是batchSize的整数倍！");
		...
		while (count <= iterations) {
			...
			lastLoss = loss;
			if(loss < deviation) {
				System.out.println("ASGD训练完成！");
				break;
			}
			if (lossEqual > 5) {
				System.out.println("loss值不在改变，ASGD训练完成！");
				break;
			}
		}
		for (int i = 0; i < theta.length; i++) {
	 ...
	 * @throws Exception
	 */
	public static double[] asgdTrain2(double[][] x_data, double[] y_data, double alpha, double deviation, int batchSize, int iterations) throws Exception {
		int parameter = x_data[0].length;
		int size = y_data.length;
		if (size % batchSize != 0) {
			throw new Exception("样本数必须是batchSize的整数倍！");
